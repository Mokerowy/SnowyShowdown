...
import * as Drawing from "./drawing.js"; // Ta linia jest poprawna
...
// --- Stan Gry ---
// DODANO 'INFO' DO KOMENTARZA
let gameState = "LOADING"; // "LOADING", "MENU", "INFO", "PLAYING", "ROUND_OVER"
let numPlayers = 1;
...
function animationLoop(timestamp) {
  ...
  switch (gameState) {
    ...
    // --- NOWY STAN INFO ---
    case "INFO":
      // Używamy funkcji z 'Drawing', którą już masz
      Drawing.drawInfoScreen(ctx, Config.playerKeyMaps);
      break;
    // --- KONIEC NOWEGO STANU ---
...
function renderGame() {
  // ... (Ta funkcja pozostaje bez zmian - używa CSS) ...
...
function handleKeyDown(event) {
  ...
  if (gameState === "MENU") { 
    ...
    // --- LOGIKA MOTYWU (Klawisz "T") --- (Twoja wersja)
    if (key === "t" || code === "KeyT") {
      ...
      statusMessage.textContent = `Motyw zmieniony na: ${newTheme}`; // Dodano informację zwrotną
    }
    // --- KONIEC LOGIKI MOTYWU ---

    // --- NOWA LOGIKA INFO (Klawisz "I") ---
    if (key === "i" || code === "KeyI") {
      gameState = "INFO";
      statusMessage.textContent = "Wyświetlanie sterowania...";
    }
    // --- KONIEC NOWEJ LOGIKI ---
  ...
  // --- NOWY BLOK OBSŁUGI DLA INFO ---
  if (gameState === "INFO") {
    if (key === "escape" || code === "Escape" || key === "i" || code === "KeyI") {
      gameState = "MENU";
      statusMessage.textContent = "Wybierz tryb gry.";
    }
    return;
  }
  // --- KONIEC NOWEGO BLOKU ---
...
window.onload = async () => {
  try {
    ...
    // --- Ustaw domyślny motyw przy starcie --- (Twoja wersja)
